...
# TensorFlow and MediaPipe specific suppressions
...
def detect_pose(results):
    ...
    left_knee, right_knee = (
        landmarks[mp_holistic.PoseLandmark.LEFT_KNEE],
        landmarks[mp_holistic.PoseLandmark.RIGHT_KNEE],
    )
    left_ankle, right_ankle = (
        landmarks[mp_holistic.PoseLandmark.LEFT_ANKLE],
        landmarks[mp_holistic.PoseLandmark.RIGHT_ANKLE],
    )
    ...
    if (
        abs(left_shoulder.y - left_elbow.y) < 0.05  # Left elbow at shoulder height
        and abs(left_elbow.y - left_wrist.y) < 0.05  # Left wrist at elbow height
        and left_wrist.x < left_shoulder.x - 0.1  # Left wrist extended forward
        and right_wrist.x > right_shoulder.x  # Right wrist not forward
    ):
        return "Left arm forward!"

    # Right arm forward detection
    if (
        abs(right_shoulder.y - right_elbow.y) < 0.05  # Right elbow at shoulder height
        and abs(right_elbow.y - right_wrist.y) < 0.05  # Right wrist at elbow height
        and right_wrist.x > right_shoulder.x + 0.1  # Right wrist extended forward
        and left_wrist.x < left_shoulder.x  # Left wrist not forward
    ):
    ...
    if (
        abs(left_shoulder.y - left_elbow.y) < 0.05
        and abs(right_shoulder.y - right_elbow.y) < 0.05
        and abs(left_elbow.y - left_wrist.y) < 0.05
        and abs(right_elbow.y - right_wrist.y) < 0.05
        and abs(left_wrist.x - right_wrist.x) > shoulder_width * 1.8
    ):
    ...
    if (
        abs(left_wrist.x - right_wrist.x) < shoulder_width * 0.1
        and abs(left_wrist.y - right_wrist.y) < shoulder_width * 0.1
    ):
...
while cap.isOpened():
    ...
    cv2.setWindowProperty(
        "PhysioPlay Game", cv2.WINDOW_FREERATIO, cv2.WND_PROP_ASPECT_RATIO
    )
    cv2.imshow("PhysioPlay Game", frame_bgr)
    key = cv2.waitKey(10) & 0xFF
    if remaining_time <= 0 or (key == ord("q")):
        break
    if key == ord("s"):
        score += 1
        current_exercise = random.choice(exercises)
